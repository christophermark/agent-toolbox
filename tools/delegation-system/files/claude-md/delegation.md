...
- Workflows are opt-in — the `Workflow` tool never fires unless it was asked for. When work is genuinely pipeline-shaped (N items through the same stages, or findings that need adversarial verification), offer one instead of silently falling back to a hand fan-out. Inside a script: pass `agentType` from the routing above on every `agent()` call, since the default is this session's expensive model; give parallel writers `isolation: 'worktree'`; and iterate by editing the script and resuming with `resumeFromRunId` — workflow agents aren't reachable via SendMessage, and the Codex lane stays in your own turn.
